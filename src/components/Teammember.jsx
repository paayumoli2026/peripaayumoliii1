...
/* Gradient pairs for the initial-avatar fallback */
...
const Teammember = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6 pt-32 space-y-16 overflow-hidden bg-transparent">
      ...
      <div className="p-1.5 glass-nav inline-flex items-center gap-1 bg-white/40 backdrop-blur-md rounded-full border border-white/20 shadow-xl">
        {DEPARTMENTS.map((dept) => (
          <button
            key={dept}
            onClick={() => setActiveDept(dept)}
            className={`px-10 py-3 rounded-full text-sm font-black uppercase tracking-widest transition-all duration-300 ${activeDept === dept
        ...
        >
          {filtered.map((member, idx) => (
            <motion.a
              ...
              className="group relative flex flex-col items-center justify-center px-4 py-6 glass-card rounded-2xl
                         text-gray-900 font-black text-sm sm:text-base uppercase tracking-wide
                         transition-all duration-300
                         hover:-translate-y-1 hover:shadow-[0_12px_32px_-6px_rgba(255,92,0,0.25)]
            ...
            >
                ...
                {member.photo ? (
                  <img
                    ...
                    className="w-36 h-36 sm:w-40 sm:h-40 rounded-full object-cover
                               border-[3px] border-white/60 shadow-lg
                               group-hover:scale-105 transition-transform duration-300"
                  />
                ) : (
                  <div
                    className={`w-36 h-36 sm:w-40 sm:h-40 rounded-full
                                bg-gradient-to-br ${AVATAR_GRADIENTS[member.id % AVATAR_GRADIENTS.length]}
              ...
              {/* LinkedIn icon */}
              <svg
                className="absolute top-2 right-2 w-4 h-4 text-[#0077B5] opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                viewBox="0 0 24 24"
                fill="currentColor"
              >
